...
import { CreateBankAccount } from './../../../../helpers/apis/BankAccount';
...
const Create = () => {
    ...
    const save = async (e) => {
        ...
        if (name == "" || number == "" || ownerRuc == "" || ownerName == "") {
            ...
            const response = await CreateBankAccount(object);
            if (response.statusCode == 200) {
                setOk(true);
            } else if (response.statusCode == 409) {
                await setMessage("El número de cuenta ya se encuentra registrado a otra cuenta")
                setError(true);
            }
            else {
                await setMessage("Ocurrió un error al intentar guardar la cuenta, contacte al soporte técnico")
                setError(true);
            }
        }
    };
                            ...
                            <div className="px-4 py-5 sm:p-6">
                                <form className="space-y-8 divide-y divide-gray-200" onSubmit={save}>
                                    <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                                        <div>
                                            <div>
                                                <h3 className="text-lg leading-6 font-medium text-gray-900">Nueva Cuenta</h3>
                                                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                                    Rellena la información para crear una nueva cuenta
                                                </p>
                                            </div>
                                        </div>
                                                ...
                                                <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                                    ...
                                                    <div className="mt-1 sm:mt-0 sm:col-span-2">
                                                        <input
                                                            ...
                                                            name="model"
                                                            id="model"
                                                            onChange={(e) => { setOwnerRuc(e.target.value.toUpperCase()) }}
                                                            value={ownerRuc}
                                                            autoComplete="given-name"
                                                            className="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                                                            required
