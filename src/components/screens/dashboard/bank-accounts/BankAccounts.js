...
import { ExclamationIcon, PencilAltIcon,XCircleIcon } from "@heroicons/react/outline";
import Pagination from "../../../component/Pagination";
import {
  GetAllBankAccounts,
  DeleteBankAccount,
} from "../../../../helpers/apis/BankAccount";
...
const BankAccounts = () => {
  ...
  const Filter = (text) => {
    const result = BankAccounts.filter(
      (item) =>
        item.owner_ruc.includes(text) ||
        item.owner_name.includes(text) ||
        item.account_number.includes(text) ||
        item.bank_name.includes(text)
    );
    if (result) {
  ...
  const currentPosts = BankAccount.slice(indexOfFirstPost, indexOfLastPost);
                          ...
                          >
                            ...
                            >
                              <div className="relative flex flex-wrap items-center ">
                                <Link
                                  style={{ marginLeft: 10 }}
                                  to={`/dashboard/bank-accounts/${person.id}`}
              ...
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                <div>
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        La Cuenta esta involucrada en un proceso , por lo tanto
                        no puede eliminarse
                      </p>
                    </div>
                  </div>
