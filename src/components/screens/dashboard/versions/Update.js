...
import { UpdateVersion, GetOneVersion } from './../../../../helpers/apis/Version';
...
const Update = (props) => {
    ...
    useEffect(() => {
        const request = async () => {
            const response = await GetOneVersion(props.id);
    ...
    const update = async (e) => {
        ...
        if (content == "") {
            ...
            const response = await UpdateVersion(object);
            if (response.statusCode == 200) {
                setOk(true);
            }
            else {
                await setMessage("Ocurrió un error al intentar actualizar la versión, contacte al soporte técnico")
                setError(true);
            }
        }
    };
                            ...
                            <div className="px-4 py-5 sm:p-6">
                                <form className="space-y-8 divide-y divide-gray-200" onSubmit={update}>
                                    <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                                        <div>
                                            <div>
                                                <h3 className="text-lg leading-6 font-medium text-gray-900">Editar versión</h3>
                                                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                                    Rellena la información para actualizar la versión
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                                            <div>
                                                <h3 className="text-lg leading-6 font-medium text-gray-900">Información de la versión</h3>
                                                <p className="mt-1 max-w-2xl text-sm text-gray-500">Ingresa los datos de la versión</p>
                                            </div>
                                    ...
                                    <div className="pt-5">
                                        <div className="flex justify-end">
                                            <button
                                                onClick={() => {
                                                    navigation.push("/dashboard/version");
                                                }}
                                                type="button"
                            ...
                            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                                    ...
                                    <div className="mt-3 text-center sm:mt-5">
                                        ...
                                        <div className="mt-2">
                                            <p className="text-sm text-gray-500">
                                                La versión ha sido actualizado con éxito
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-5 sm:mt-6">
                                    <button
                                        ...
                                        onClick={async () => { await setOk(false); navigation.push('/dashboard/version'); }}
                                    >
                                        Aceptar
                                    </button>
                                </div>
