...
import { GetOneAdmin, UpdateAdmin } from "./../../../../helpers/apis/Admin";
...
const Update = (props) => {
  ...
  useEffect(() => {
    const request = async () => {
      const admin = await GetOneAdmin(props.id);
      if (admin.statusCode == 200) {
        setName(admin.data.fullname);
        setUserId(admin.data.User.id);
        setUsername(admin.data.User.username);
        setNumber(admin.data.companyNumber);
        setName(admin.data.fullname);
        setContact(admin.data.contact);
        setPhone(admin.data.phone);
        setAddress(admin.data.address);
        setLat(admin.data.latitude);
        setLng(admin.data.longitude);
      }
    };
    request();
  }, []);
  ...
  const update = async (e) => {
    ...
    if (
      ...
      if (response.statusCode == 200) {
        ...
        await setMessage(
          "El usuario ya se encuentra asignado a un administrador"
        );
        setError(true);
      } else {
        await setMessage(
          "Ocurrió un error al intentar actualizar el administrador, contacte al soporte técnico"
        );
        setError(true);
      }
    }
                ...
                >
                  <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                    <div>
                      <div>
                        ...
                        <p className="mt-1 max-w-2xl text-sm text-gray-500">
                          Rellena la información para actualizar la terminal
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                      <div>
                        ...
                        <p className="mt-1 max-w-2xl text-sm text-gray-500">
                          Ingresa los datos de la terminal
                        </p>
                      </div>
                  ...
                  <div className="pt-5">
                    <div className="flex justify-end">
                      <button
                        onClick={() => {
                          navigation.push("/dashboard/administrators");
                        }}
                        type="button"
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        El administrador ha sido actualizada con éxito
                      </p>
                    </div>
                  </div>
                </div>
