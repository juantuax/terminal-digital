...
import { ExclamationIcon, PencilAltIcon, XCircleIcon, CheckCircleIcon } from '@heroicons/react/outline';
import Pagination from '../../../component/Pagination';
import { GetAllClients, GetAllClientsActive, DeleteClient, ActiveClient } from './../../../../helpers/apis/Client';
...
const Clients = () => {
    ...
    useEffect(() => {
        const request = async () => {
            ...
            const responseActive = await GetAllClientsActive();
            if (responseActive.statusCode == 200) {
                setClient(responseActive.data);
            }
        };
        request();
    }, []);
    ...
    const Delete = async () => {
        ...
        if (response.statusCode == 200) {
            const responseActive = await GetAllClients();
            if (responseActive.statusCode == 200) {
                let arrayActive = [];
                ...
                setClients(responseActive.data);
                setClient(arrayActive)
                setOpen(false);
            } else {
                setOpen(false);
            }
    ...
    const Active = async () => {
        const response = await ActiveClient(clientId)
        if (response.statusCode == 200) {
            const request = async () => {
                const response = await GetAllClients();
                if (response.statusCode == 200) {
    ...
    const Filter = async (text) => {
        if (text == "") {
            ...
            const result = clients.filter(item => item.codeClient.includes(text) || item.fullname.includes(text));
            if (result) {
                setClient(result);
            }
        }
    ...
    const FilterActive = (text) => {
        const result = clients.filter(item => (item.active == 0 ? "ELIMINADO" : "ACTIVO").includes(text));
        if (result) {
            setClient(result);
        }
        else {
                ...
                <div className="bg-white px-4 py-5 border-b border-gray-200 sm:px-6">
                    <div className="-ml-4 -mt-2 flex items-center justify-between flex-wrap sm:flex-nowrap">
                        <div className="ml-4 mt-2">
                            <h3 className="text-lg leading-6 font-medium text-gray-900">Clientes</h3>
                        </div>
                                                            ...
                                                            <tr key={person.phone} className={personIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                                <td style={{ width: 200 }} >
                                                                    <div className="relative flex flex-wrap items-center ">
                                                                        <Link style={{ marginLeft: 10 }} to={`/dashboard/clients/${person.id}`} className="text-indigo-600 hover:text-indigo-900"><PencilAltIcon className="h-6 w-6" aria-hidden="true" data-tip="Editar" />
                                                                            {/* <ReactTooltip /> */}
                                                                        </Link>
                                                                        <Link
                                                                            style={{ marginLeft: 10 }}
                                                                            onClick={() => { setClientId(person.id); setOpen(true) }}
                                                                            className="text-gray-600 hover:text-gray-900"><XCircleIcon className="h-6 w-6" aria-hidden="true" data-tip="Eliminar" />
                                                                            {/* <ReactTooltip /> */}
                                                                        </Link>
                                                                    </div>
                                                                </td>
                                                                <td style={{ width: 200 }} className="px-6 py-4 text-sm font-medium text-gray-900">{person.codeClient}</td>
                                                                <td style={{ maxWidth: 200 }} className="px-6 py-4 text-sm text-gray-500">{person.fullname}</td>
                                                            </tr>
                                                            :
                                                            <tr key={person.phone} className={personIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                                <td style={{ width: 200 }} >
                                                                    <div className="relative flex flex-wrap items-center justify-rigth lg:rigth">
                                                                        <Link
                                                                            style={{ marginLeft: 10 }}
                                                                            onClick={() => { setClientId(person.id); setActive(true) }}
                                                                            className="text-gray-600 hover:text-gray-900"><CheckCircleIcon className="h-6 w-6" aria-hidden="true" data-tip="Activar" />
                                                                            {/* <ReactTooltip /> */}
                                                                        </Link>
                                                                    </div>
                                                                </td>
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        ...
                                        <div className="mt-2">
                                            <p className="text-sm text-gray-500">
                                                Esta seguro que desea Activar el cliente?
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                                <div>
                                    ...
                                    <div className="mt-3 text-center sm:mt-5">
                                        ...
                                        <div className="mt-2">
                                            <p className="text-sm text-gray-500">
                                                El cliente esta involucrado en un proceso , por lo tanto no puede eliminarse
                                            </p>
                                        </div>
                                    </div>
                                </div>
