...
import { UpdateClient, GetOneClient } from './../../../../helpers/apis/Client';
...
const Update = (props) => {
    ...
    useEffect(() => {
        const request = async () => {
            const response = await GetOneClient(props.id);
            if (response.statusCode == 200) {
                setClient(response.data.codeClient);
    ...
    const update = async (e) => {
        ...
        if (name == "" || codeClient == "") {
            ...
            const response = await UpdateClient(object);
            if (response.statusCode == 200) {
                setOk(true);
            }
            else if (response.statusCode == 409) {
                await setMessage("El número de identificación ya se encuentra registrado a otro cliente")
                setError(true);
            }
            else {
                await setMessage("Ocurrió un error al intentar actualizar el cliente, contacte al soporte técnico")
                setError(true);
            }
        }
    };
                                ...
                                <form className="space-y-8 divide-y divide-gray-200" onSubmit={update}>
                                    <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                                        <div>
                                            <div>
                                                ...
                                                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                                    Rellena la información para actualizar el cliente
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                                            <div>
                                                ...
                                                <p className="mt-1 max-w-2xl text-sm text-gray-500">Ingresa los datos del cliente</p>
                                            </div>
                                    ...
                                    <div className="pt-5">
                                        <div className="flex justify-end">
                                            <button
                                                onClick={() => {
                                                    navigation.push("/dashboard/clients");
                                                }}
                                                type="button"
                            ...
                            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                                ...
                                <div className="mt-5 sm:mt-6">
                                    <button
                                        ...
                                        onClick={async () => { await setOk(false); navigation.push('/dashboard/clients'); }}
                                    >
                                        Aceptar
                                    </button>
                                </div>
